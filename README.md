...
All the dependencies will already be in the Docker container.
...
"post_processing":{
    ...
    "tritium_diffusion":{
      ...
      "output_file":"Examples/Cube_tuto/post-processing-tritium-diffusion.csv"
      }
```
Note : the output files must be .csv.
### Step 6 : Run the simulation
To be able to run 3-MOT, you will need to enable FEniCS in the Docker container. Run the Docker image with:
```sh
$ fenicsproject run
```

To run the simulation with the parameters above, you need to specify the path of the JSON file with:
```sh
$ python3 3-MOT.py "Examples/Cube-tuto/MOT_parameters_cube_diffusion.json"
```
or
```sh
$ python3 3-MOT.py Examples/Cube-tuto/MOT_parameters_cube_diffusion.json
```
### Step 7 : Visualisation & Post-computing
At the end of the computation, you shall find several files in ```Examples/Cube_tuto/Solution``` that can be visualise with any post-processing software such as [Paraview](#https://www.paraview.org/) or [Visit](#https://wci.llnl.gov/simulation/computer-codes/visit).

The post-computations defined in [Step 5.2](#Step-5.2-:-Define-the-post-processing-computations-(optionnal)) can be found in ```Examples/Cube_tuto/post-processing-tritium-diffusion.csv``` and  ```Examples/Cube_tuto/post-processingHT.csv```.
